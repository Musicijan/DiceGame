...
function reset(sendToWS = true) {
   ...
   if (sendToWS) {
      ws.send(JSON.stringify({ command: "reset_game" }));
   }
}

function roll() {
   if (!verifyRoll()) return false;
...
function showAllScores(allScores) {
